...
│   │   ├── apps/
│   │   │   ├── __init__.py
│   │   │   ├── conversation_app.py   # 智能对话
│   │   │   ├── emotion_app.py        # 情绪分析
│   │   │   ├── sas_app.py            # SAS 评估
│   │   │   ├── relaxation_app.py     # 放松训练
│   │   │   ├── user_app.py           # 用户管理
│   │   │   ├── admin_app.py          # 管理员功能
│   │   │   └── statistics_app.py     # 统计面板
│   │   ├── db/
│   │   │   ├── __init__.py
...
├─────────────────────────────────────────┤
│         api/apps/ (业务逻辑层)            │
│  conversation | emotion | sas | user ... │
├─────────────────────────────────────────┤
│         api/db/ (数据持久层)              │
│  Peewee ORM + SQLite                    │
├─────────────────────────────────────────┤
│       api/settings.py (模型层)            │
│  camel-ai → OpenAI SDK → LLM API        │
└─────────────────────────────────────────┘
前端：Gradio 5.x Blocks，自定义 CSS 样式
后端：Python 模块化架构，api/apps 作为业务逻辑层
模型调用：camel-ai 封装 OpenAI SDK，支持 OpenAI 兼容接口
数据存储：Peewee ORM + SQLite 本地数据库
八、核心依赖
依赖	用途	版本
Gradio	Web UI 框架	5.38
camel-ai	LLM 模型封装	0.2.72
openai	API 客户端	1.97
peewee	ORM 数据库	3.18
matplotlib	图表生成	3.10
cryptography	AES 加解密	45.0
loguru	日志管理	0.7.3
九、已知问题与注意事项
代理绕过：代码中已通过 monkey-patch 强制 httpx trust_env=False，避免系统代理干扰
默认密码：管理员默认密码 admin@123，首次登录后建议立即修改
队列错误：Gradio 5.x 在沙箱环境中的 WebSocket 队列错误（ERR_ABORTED queue/data）不影响核心功能
SAS 计分：量表包含 5 道反向计分题（第 5、9、13、17、19 题），代码已自动处理
